---
name: pmstack-eval
description: Help a product manager define success and create an evaluation suite for a model or agent, including long-running AI teammates. Teaches tasks, trials, outcomes, graders, reference solutions, case mix, and evaluation validity using Anthropic terminology. Produces JSON for pmstack's eval-harness.mjs, with explicit examples and integration requirements. Use when defining AI quality, designing agent tests, choosing graders, or investigating whether an eval fairly measures customer value.
---
