...
5. **`/vibe-test`** — Anthropic's layer-1 ritual: read raw transcripts before writing the eval. Tutorial uses 5 mock transcripts at `examples/walkthrough-code-review/transcripts/`.
6. **`/sprint`** — chain metrics → eval → run-eval → brief.
7. **`/transcript-review`** — Anthropic's Step 6 ritual: diagnose every failed eval trial as *model mistake / grader mistake / task-spec error*.
8. **`/launch-readiness` + `/lint`** — gate routines before shipping.
9. **Schedule the recurring routines** — `/weekly`, `/eval-drift`, `/lint` on `/loop 7d`.
