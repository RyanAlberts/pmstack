...
2. **Settings → Skills → Upload skill** → upload each folder from [`claude-skills/`](./claude-skills/). All 20, or pick the 4–5 you'll use most.
...
Commands arrive namespaced — `/pmstack:eval`, `/pmstack:prd`, `/pmstack:weekly` — and all 20 skills auto-activate on plain English ("write a PRD from this customer quote"). Update later with `/plugin`, no re-install.
